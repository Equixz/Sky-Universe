...
Citizen.CreateThread(function()
	...
	Citizen.Wait(0)
			-- SALGSROOM
			if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), salg.x, salg.y, salg.z, true ) < 6 then
				if farm == false then 
					DrawText3Ds(salg.x,salg.y,salg.z+0.4, "~w~Tryk [~g~E~w~] for at ~g~arbejde~w~.", 3.0, 7)	
					...
					DrawText3Ds(salg.x,salg.y,salg.z+0.4, "~w~Tryk [~g~E~w~] for at ~r~stoppe~w~.", 3.0, 7)	
						if IsControlJustPressed(1, key) then
							local spiller = PlayerPedId()
			...
			-- Skunk FARM
			elseif GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), sku1.x, sku1.y, sku1.z, true ) < 6 then
				if farm == false then 
			...
			elseif GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), sku2.x, sku2.y, sku2.z, true ) < 6 then
				if farm == false then 
					DrawText3Ds(sku2.x,sku2.y,sku2.z+0.4, "~w~Tryk [~g~E~w~] for at ~g~tørre~w~.", 3.0, 7)	
						if IsControlJustPressed(1, key) then
							local spiller = PlayerPedId()
							TaskStartScenarioInPlace(spiller, "PROP_HUMAN_PARKING_METER")
							FreezeEntityPosition(spiller,true)	
