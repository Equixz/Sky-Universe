...
RegisterNetEvent( 'longboard' )
AddEventHandler( 'longboard', function()
	RCCar.Start()
end)
	...
	if distanceCheck <= Config.LoseConnectionDistance then
		...
		if distanceCheck <= 1.5 then
			...
			if not Attached then
				DrawText3Ds(GetEntityCoords(RCCar.Entity).x, GetEntityCoords(RCCar.Entity).y, GetEntityCoords(RCCar.Entity).z + 0.5, "[E] Saml op")
			end
		end
	end
