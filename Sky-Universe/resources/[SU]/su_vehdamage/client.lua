...
RegisterNetEvent('kaz:sendbil')
AddEventHandler('kaz:sendbil', function(skade, model, player)
Citizen.Wait(100)
local spiller = GetPlayerPed(-1)
local bil = GetVehiclePedIsIn(spiller, false)
if skade ~= nil then 
skade1 = skade+0.0
	if bil == model then 
		if player == spiller then 
			bilskade1 = skade1
			nybil = true
			Citizen.Wait(1000)
			nybil = false
...
Citizen.CreateThread(function()
	while true do
		...
		if isPedDrivingAVehicle() then
			...
			lastVehicle=vehicle
			if nybil == true then
				SetVehicleEngineHealth(vehicle, bilskade1)
				healthEngineLast = bilskade1
			end
		else
			if pedInSameVehicleLast == true then
