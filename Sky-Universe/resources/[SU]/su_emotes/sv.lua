...
RegisterCommand("cam", function(source, args, raw)
    local src = source
    TriggerClientEvent("su_emotes:mobil", src)
end)

RegisterServerEvent('kaz:particles')
AddEventHandler('kaz:particles', function(ped,dict,par,settings,type,stoptime)
    TriggerClientEvent('kaz:startparticles',-1,ped,dict,par,settings,type,stoptime)
end)

RegisterServerEvent('kaz_emotes:particles')
AddEventHandler('kaz_emotes:particles', function(ped,dict,par,settings,type,stoptime)
    TriggerClientEvent('kaz_emotes:startparticles',-1,ped,dict,par,settings,type,stoptime)
end)
