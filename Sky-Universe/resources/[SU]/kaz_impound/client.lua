...
RegisterCommand('impound', function(source)
			...
			if plate2 ~= nil then 
				...
				if plate ~= nil then
					...
					TriggerEvent("pNotify:SendNotification",{text = "Ingen bil fundet.",type = "error",timeout = (5000),layout = "centerLeft",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
				end
			else
				TriggerEvent("pNotify:SendNotification",{text = "Ingen bil fundet.",type = "error",timeout = (5000),layout = "centerLeft",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
			end
...
local spiller = GetPlayerPed(-1)
	if user_id == spiller then
		...
		TriggerEvent("pNotify:SendNotification",{text = "Køretøj impounded!",type = "error",timeout = (5000),layout = "centerLeft",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
