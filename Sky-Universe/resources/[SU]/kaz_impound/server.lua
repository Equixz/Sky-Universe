...
if checker == nil then 
TriggerClientEvent("pNotify:SendNotification", source,{text = "Du skal indtaste nummerpladen!", type = "success", queue = "global", timeout = 5000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
end
	...
	TriggerClientEvent("pNotify:SendNotification", source,{text = "Ingen adgang!", type = "success", queue = "global", timeout = 5000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
	end
end)
...
PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/606747044308058122/vUK9sj_FtaeXQ_rgA6VqV77QQztOs-KvCmgxrN7r3Pq9nQzLeVerQtFgwqJ1v84ofjjO', function(err, text, headers) end, 'POST', json.encode({username = "Impounder", content = "**ID: "..user_id.."**  beslaglagde ID: **"..nuser_id.."´s** køretøj med navnet "..vehiclename.."."}), { ['Content-Type'] = 'application/json' })
end)	
...
local source = source
local source2 = vRP.getUserId({source})
PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/606747044308058122/vUK9sj_FtaeXQ_rgA6VqV77QQztOs-KvCmgxrN7r3Pq9nQzLeVerQtFgwqJ1v84ofjjO', function(err, text, headers) end, 'POST', json.encode({username = "Impounder", content = "**ID: "..source2.."**  slettede et køretøj af typen: "..name.."."}), { ['Content-Type'] = 'application/json' })
end)	
...
impounded = 1
		MySQL.query("vRP/kaz_getnumberimp", {user_id = user_id, vehicle_impound = impounded}, function(data)
			for index, data in ipairs(data) do
				...
				caramount = value1
				if caramount == 0 then 
				TriggerClientEvent("pNotify:SendNotification", source,{text = "Du har ingen impounded køretøjer.", type = "success", queue = "global", timeout = 5000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
				else
					price = (25000*caramount)
					if vRP.tryFullPayment({user_id,price}) then 
						TriggerClientEvent("pNotify:SendNotification", source,{text = "Du har nu købt "..caramount.." køretøj(er) tilbage for "..price.."kr.", type = "success", queue = "global", timeout = 10000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
						MySQL.query("vRP/kaz_findimpounds", {user_id = user_id, impounded = impounded}, function(impoundedcars, affected)
							for k,v in pairs(impoundedcars) do 
								...
								impoundnew = 0
								PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/606747044308058122/vUK9sj_FtaeXQ_rgA6VqV77QQztOs-KvCmgxrN7r3Pq9nQzLeVerQtFgwqJ1v84ofjjO', function(err, text, headers) end, 'POST', json.encode({username = "Impounder", content = "**ID: "..user_id.."**  købte bilen "..vehiclenamenow.." tilbage fra impound."}), { ['Content-Type'] = 'application/json' })
					...
					TriggerClientEvent("pNotify:SendNotification", source,{text = "Du har ikke råd", type = "success", queue = "global", timeout = 5000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
					end
				end	
end)	end)
