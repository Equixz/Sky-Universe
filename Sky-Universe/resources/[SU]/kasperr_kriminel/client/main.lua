...
vRPclient = Tunnel.getInterface("vRP", "kasperr_kriminel")
  ...
  while true do
    ...
    for _, item in pairs(Config.Locations) do
      if(Vdist(pos.x, pos.y, pos.z, item.x, item.y, item.z+1) < 15.0)then
        DrawMarker(27, item.x, item.y, item.z+1, 0, 0, 0, 0, 0, 0, 1.001, 1.0001, 1.0001, 0, 155, 255, 200, 0, 0, 0, 1)
...
RegisterNUICallback("selectJob", function(data)
  if data.group ~= nil then
    TriggerServerEvent("kasperr_kriminel:selectJob", data.group)
  end
end)

RegisterNetEvent("kasperr_kriminel:notification")
AddEventHandler("kasperr_kriminel:notification", function(message)
