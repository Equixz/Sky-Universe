...
function IsPlayerInPolmav()
	...
	if IsVehicleModel(vehicle, GetHashKey("EAS350EA")) then 
		return true
	elseif IsVehicleModel(vehicle, GetHashKey("EAS350EP")) then 
		return true
	else 
		return false
	end
...
-- Script starts here
local helicam = false
local ambumav_hash = GetHashKey("ambumav")
local fov = (fov_max+fov_min)*0.5
local vision_state = 0 -- 0 is normal, 1 is nightmode, 2 is thermal vision
Citizen.CreateThread(function()
	while true do
        Citizen.Wait(0)
		if IsPlayerInAmbumav() then
			local lPed = GetPlayerPed(-1)
			local heli = GetVehiclePedIsIn(lPed)
...
function IsPlayerInAmbumav()
	local lPed = GetPlayerPed(-1)
	local vehicle = GetVehiclePedIsIn(lPed)
	return IsVehicleModel(vehicle, ambumav_hash)
end
