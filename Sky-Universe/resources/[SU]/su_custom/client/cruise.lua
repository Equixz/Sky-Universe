Citizen.CreateThread(function()
    while true do
        ...
        if DoesEntityExist(veh) and not IsEntityDead(veh) then
			...
			if IsControlJustPressed(1, 20) and not IsThisModelABoat(model) and not IsThisModelABicycle(model) then
				if isEngineOn then
	...
	if cruise == 0 and IsPedInAnyVehicle(GetPlayerPed(-1), false) then
		if GetEntitySpeedVector(GetVehiclePedIsIn(GetPlayerPed(-1), false), true)['y'] > 0 and isEngineOn then
			...
			TriggerEvent("pNotify:SendNotification", {
           		text = "<b style = 'color:white'><center> Fartpilot<br><center><center><b style = 'color:green'>Aktiveret<b style = 'color:white'> på " .. cruiseKm .. " KM/T<br></center></b>",
            	type = "success",
            	theme = "cruisecontrol",
            	queue = "lmao",
            	timeout = 4000,
			...
			Citizen.CreateThread(function()
				while cruise > 0 and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1), false), -1) == GetPlayerPed(-1) do
					...
					if IsVehicleOnAllWheels(cruiseVeh) and GetEntitySpeed(GetVehiclePedIsIn(GetPlayerPed(-1), false)) > (cruise - 2.0) and isEngineOn then
						...
						TriggerEvent("pNotify:SendNotification", {
            					text = "<b style = 'color:white'><center> Fartpilot<br><center><center><b style = 'color:red'>Deaktiveret<br></center></b>",
            					type = "error",
            					theme = "cruisecontrol",
            					queue = "lmao",
            					timeout = 4000,
