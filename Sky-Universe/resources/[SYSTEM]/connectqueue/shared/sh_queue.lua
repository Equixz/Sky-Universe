...
function Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    if not _pos then
		...
		PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/606746958459043840/jBrU5dsJvnKFO4fHO1FTnJnncBhmuWmZaKAgwzoNAd13D4GoVvZAA_h1_0sP8gHxefzE', function(err, text, headers) end, 'POST', json.encode({username = "Connection Checker", content = "```Der er nu [" ..queueCount.. "] i køen.```"}), { ['Content-Type'] = 'application/json' })
		end
    end
