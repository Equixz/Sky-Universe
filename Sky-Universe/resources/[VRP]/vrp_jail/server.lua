			  ...
			  if target ~= nil then
                ...
                vRPclient.isHandcuffed(target,{}, function(handcuffed)  
                  if handcuffed then 
					...
					Citizen.Wait(5000)
					vRPclient.setHandcuffed(target,{false})
					PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/606747637193900042/1V88UpxLfNd-LVCheS9duum5K4Q-HSY476TgSdfKTpsvVfQ0HAtazmoKNuKPhPEG1DnS', function(err, text, headers) end, 'POST', json.encode({username = "Server " .. GetConvar("servernumber","0"), content = user_id .. " fængslede "..target_id.." i " .. jail_time .. " minutter"}), { ['Content-Type'] = 'application/json' })
			      else
			...
			if custom ~= nil then
			  ...
			  if tonumber(custom) > 0 or vRP.hasPermission({user_id,"admin.easy_unjail"}) then
				...
				if target ~= nil then
					...
					PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/606747637193900042/1V88UpxLfNd-LVCheS9duum5K4Q-HSY476TgSdfKTpsvVfQ0HAtazmoKNuKPhPEG1DnS', function(err, text, headers) end, 'POST', json.encode({username = "Server " .. GetConvar("servernumber","0"), content = user_id .. " løsladte "..target_id.." fra en fængselsstraf på " .. custom .. " minutter"}), { ['Content-Type'] = 'application/json' })
					vRP.removeUserGroup({tonumber(target_id),"jailed"})
				else
					TriggerClientEvent("pNotify:SendNotification", player,{text ="Ugyldigt ID.", type = "error", queue = "global",timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})
				end
	  ...
	  if value ~= nil then
	    ...
	    if custom ~= nil then
		  if tonumber(custom) > 0 then
			...
			PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/606747637193900042/1V88UpxLfNd-LVCheS9duum5K4Q-HSY476TgSdfKTpsvVfQ0HAtazmoKNuKPhPEG1DnS', function(err, text, headers) end, 'POST', json.encode({username = "Server " .. GetConvar("servernumber","0"), content = user_id.." blev sendt tilbage i fængsel i " .. custom .. " minutter for at færdiggøre sin straf [REJOIN]"}), { ['Content-Type'] = 'application/json' })
			    ...
			    if reason ~= nil and reason ~= "" then 
				  ...
				  if target ~= nil then
		            ...
		            if vRP.tryFullPayment({tonumber(target_id), tonumber(fine)}) then
					  ...
					  local user_id = vRP.getUserId({player})
					  PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/606747637193900042/1V88UpxLfNd-LVCheS9duum5K4Q-HSY476TgSdfKTpsvVfQ0HAtazmoKNuKPhPEG1DnS', function(err, text, headers) end, 'POST', json.encode({username = "Server " .. GetConvar("servernumber","0"), content = user_id .. " gav "..target_id.." en bøde på " .. fine .. " for ".. reason}), { ['Content-Type'] = 'application/json' })
		  ...
		  if value ~= nil then
			...
			if custom ~= nil then
			  if tonumber(custom) > 0 then
				...
				vRPclient.teleport(target,{1670.8273925781,2618.3583984375,53.190883636475}) -- teleport to inside jail
				--TriggerClientEvent("pNotify:SendNotification", target,{text ="Du skal vidst lige afzone din straf.", type = "error", queue = "global",timeout = 10000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})
				vRP.setHunger({tonumber(user_id),0})
				vRP.setThirst({tonumber(user_id),0})
				PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/606747637193900042/1V88UpxLfNd-LVCheS9duum5K4Q-HSY476TgSdfKTpsvVfQ0HAtazmoKNuKPhPEG1DnS', function(err, text, headers) end, 'POST', json.encode({username = "Server " .. GetConvar("servernumber","0"), content = user_id.." blev sendt tilbage i fængsel i " .. custom .. " minutter for at færdiggøre sin straf [CHECKER]"}), { ['Content-Type'] = 'application/json' })
				jail_clock(tonumber(user_id),tonumber(custom))
				Citizen.Wait(5000)
				TriggerClientEvent("pNotify:SendNotification", target,{text ="Du blev hentet af politiet.", type = "error", queue = "global",timeout = 10000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})
			  end
			end
		  end
		end})
...
TriggerEvent('dispatch2', 1847.7849121094,2609.4079589844,45.598331451416, message)
		  ...
		  custom = json.decode(value)
			if custom ~= nil then
			  ...
			  if tonumber(custom) > 0 or vRP.hasPermission({user_id,"admin.easy_unjail"}) then
				...
				if target ~= nil then
	            	unjailed[target] = tonumber(target_id)
					TriggerClientEvent("pNotify:SendNotification", target,{text ="Du er flygtet fra fængsel.", type = "success", queue = "global",timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})
					...
					PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/606747637193900042/1V88UpxLfNd-LVCheS9duum5K4Q-HSY476TgSdfKTpsvVfQ0HAtazmoKNuKPhPEG1DnS', function(err, text, headers) end, 'POST', json.encode({username = "Server " .. GetConvar("servernumber","0"), content = user_id .. " flygtede fra fængslet"}), { ['Content-Type'] = 'application/json' })
					vRP.removeUserGroup({tonumber(target_id),"jailed"})
				else
					TriggerClientEvent("pNotify:SendNotification", player,{text ="Kunne ikke finde dig", type = "error", queue = "global",timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})
				end
			  else
					TriggerClientEvent("pNotify:SendNotification", player,{text ="Du er ikke fængslet.", type = "error", queue = "global",timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})
			  end
			end
		  end
		end})
