			...
			if(theTrailer ~= nil)then
				if not (IsEntityDead(theTrailer)) then
					...
					if (ok) then
						...
						trucker_drawTxt("~r~Du har brug for en LASTBIL og TRAILER for at færdiggøre din leverance!",1,1,0.5,0.9,0.8,255,255,255,255)
					end
					if(isInVehicle == false)then
						trucker_drawTxt("~r~Du skal bruge en LASTBILog TRAILER for at færdiggøre din leverance!",1,1,0.5,0.9,0.8,255,255,255,255)
					elseif(isInVehicle == true)then
						if not IsEntityAttached(theTrailer) then
							trailerBlip = AddBlipForEntity(theTrailer)
							SetBlipSprite(trailerBlip, 479)
							SetBlipColour(trailerBlip, 2)
							trucker_drawTxt("~r~Du skal bruge en LASTBIL og TRAILER for at færdiggøre din leverance!",1,1,0.5,0.9,0.8,255,255,255,255)
						else
							...
							trucker_drawTxt("~g~Sted: ~b~"..destinationName,1,1,0.5,0.8,0.8,255,255,255,255)
							trucker_drawTxt("~g~Last: ~b~"..destinationType,1,1,0.5,0.84,0.8,255,255,255,255)
							trucker_drawTxt("~g~Distance: ~r~"..(distanceLeft/1000).." ~b~KM ~w~| ~g~Trailer status: ~r~"..GetEntityHealth(theTrailer).." ~b~HP",4,1,0.5,0.87,0.5,255,255,255,255)
						end
					end
				else
					finishJob()
					vRP.notify({"~w~[LASTBIL] ~r~Jobbet var aflyst!"})
...
CreateThread(function()
	while true do
		...
		if(GetDistanceBetweenCoords(pos.x, pos.y, pos.z, x, y, z) < 40.0)then
			DrawText3D(x,y,z+0.4, "~b~Logistik", 2.0, 1)
			DrawText3D(x,y,z+0.15, "~g~Transport af importet last", 1, 7)
			...
			if(GetDistanceBetweenCoords(pos.x, pos.y, pos.z, x, y, z) < 5)then
				...
				if (incircle == true) then
					...
					if (hasTruck == 1) then
						if(inJob == false)then
							trucker_drawTxt("~y~[LASTBIL] ~g~Tryk ~r~[E] ~g~for leverings menuen!",1,1,0.5,0.8,0.8,255,255,255,255)
						else
							trucker_drawTxt("~y~[LASTBIL] ~r~Du har allerede et aktivt job!",1,1,0.5,0.74,0.8,255,255,255,255)
						end	
						if(IsControlJustReleased(1, 51)) and (inJob == false)then
							TriggerServerEvent('openTruckerJobs')
						end
					else
						trucker_drawTxt("~y~[LASTBIL] ~r~Du skal bruge din egen lastbil!",1,1,0.5,0.8,0.8,255,255,255,255)
					end
				end
