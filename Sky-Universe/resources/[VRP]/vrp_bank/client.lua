...
local atms = {
  ...
  {name="Hæveautomat", id=277, x=-1570.7247314453, y=-3013.3657226563, z=-74.40616607666}
}
      ...
      if(IsNearBank() or IsNearATM()) and not (GetEntityHealth(GetPlayerPed(-1)) == 105) then
		if IsNearBank() then
			...
			elseif clear == false then 
				drawTxt('~r~Banken er lukket - Kontakt politiet', 0, 1, 0.5, 0.8, 0.6, 255, 255, 255, 255) 
			end
		else 
...
RegisterNetEvent("kaz:vrpbankshutdown")
AddEventHandler("kaz:vrpbankshutdown", function()
clear = false
end)

RegisterNetEvent("kaz:vrpbankopen")
AddEventHandler("kaz:vrpbankopen", function()
clear = true
end)
