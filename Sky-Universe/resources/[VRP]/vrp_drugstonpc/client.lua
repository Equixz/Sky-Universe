          ...
          if IsPedInAnyVehicle(ped) == false then
            ...
            if pedType ~= 28 and IsPedAPlayer(ped) == false then
              ...
              local pedstatus = GetEntityCoords(ped)
                DrawText3Ds(pedstatus.x,pedstatus.y,pedstatus.z+0.25, "~w~Tryk [~g~H~w~] for at sælge.", 3.0, 7)  
...
while true do
  ...
  if selling then
    ...
    if distance > 6 then
      exports.pNotify:SetQueueMax("left", 1)
      exports.pNotify:SendNotification({
        text = "Forhandling afbrudt, du for langt væk.",
        type = "error",
        timeout = math.random(100, 3500),
        layout = "centerLeft",
        queue = "left"
