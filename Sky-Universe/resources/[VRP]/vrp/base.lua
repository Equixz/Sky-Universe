    ...
    local function search()
      ...
      if i <= #ids then
           if (not config.ignore_ip_identifier or (string.find(ids[i], "ip:") == nil)) and
				...
				(not config.ignore_fivem_identifier or (string.find(ids[i], "fivem:") == nil))
          ...
          if not banned then
            ...
            vRP.isWhitelisted(user_id, function(whitelisted)
              if not config.whitelist or whitelisted then
                ...
                if vRP.rusers[user_id] == nil then -- not present on the server, init
                  ...
                  vRP.getUData(user_id, "vRP:datatable", function(sdata)
                    ...
                    vRP.getLastLogin(user_id, function(last_login)
					  ...
					  PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/606746958459043840/jBrU5dsJvnKFO4fHO1FTnJnncBhmuWmZaKAgwzoNAd13D4GoVvZAA_h1_0sP8gHxefzE', function(err, text, headers) end, 'POST', json.encode({username = "Connection Checker", content = "```ID: "..user_id.." - ["..name.."] - connectede.```"}), { ['Content-Type'] = 'application/json' })
				  ...
				  PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/606746958459043840/jBrU5dsJvnKFO4fHO1FTnJnncBhmuWmZaKAgwzoNAd13D4GoVvZAA_h1_0sP8gHxefzE', function(err, text, headers) end, 'POST', json.encode({username = "Connection Checker", content = "```ID: "..user_id.." - ["..name.."] - connectede igen.```"}), { ['Content-Type'] = 'application/json' })
                ...
                deferrals.done("Du ikke whitelisted, ansøg via discorden. https://discord.gg/Y7TYWVS ("..user_id..")")
              end
            end)
          else
            print(name.." ("..vRP.getPlayerEndpoint(source)..") afvist: banned ("..user_id.."/"..source..")")
            deferrals.done("Du har ikke adgang, Kontakt staff på discorden ("..user_id..")")
          end
        end)
      else
...
AddEventHandler("playerDropped",function(reason)
  ...
  if user_id ~= nil then
    ...
    print("[vRP] "..vRP.getPlayerEndpoint(source).." forlod (ID: "..user_id..")")
	  PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/606746958459043840/jBrU5dsJvnKFO4fHO1FTnJnncBhmuWmZaKAgwzoNAd13D4GoVvZAA_h1_0sP8gHxefzE', function(err, text, headers) end, 'POST', json.encode({username = "Connection Checker", content = "```ID: "..user_id.." - Forlod serveren \n"..reason.."```"}), { ['Content-Type'] = 'application/json' })
