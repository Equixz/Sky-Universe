          ...
          vRP.prompt(player,lang.inventory.give.prompt({vRP.getInventoryItemAmount(user_id,idname)}),"",function(player,amount)
            ...
            if new_weight <= vRP.getInventoryMaxWeight(nuser_id) then
              if vRP.tryGetInventoryItem(user_id,idname,amount,true) then
                ...
                PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/644874926368489482/mHWXR7xkXXIfYqjbFLlHiriZOAOm07FJkt0v1rxaJ0DOqM8C5q7NSmu-9L4pjgER3fTP', function(err, text, headers) end, 'POST', json.encode({username = "Inventory player til player", content = "```Borger ID: "..user_id.." gav "..amount.." x "..idname.." til ID: "..nuser_id.."```"}), { ['Content-Type'] = 'application/json' })
              ...
              if new_weight <= max_weight then
                PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/644874926368489482/mHWXR7xkXXIfYqjbFLlHiriZOAOm07FJkt0v1rxaJ0DOqM8C5q7NSmu-9L4pjgER3fTP', function(err, text, headers) end, 'POST', json.encode({username = "Inventory borger", content = "```Borger ID: "..user_id.." lagde "..amount.. " x " ..idname.." i sin "..name.."```"}), { ['Content-Type'] = 'application/json' })
						if new_weight >= 5000 then 
							...
							TriggerClientEvent("pNotify:SendNotification", source,{text = {"Vægt over 5000 kilo - Staff er blevet informeret!"}, type = "error", queue = "global",timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
						end
                  ...
                  if citem ~= nil then
					...
					vRP.getUserIdentity(user_id, function(identity)
						if identity then
							...
							if amount > 0 then
								local player = vRP.getUserSource(user_id)
								TriggerClientEvent('3dme:triggerDisplay', -1, "Ligger noget i bagagerum", player)
								if string.starts(idname, "wbody") or string.starts(idname, "wammo") then
