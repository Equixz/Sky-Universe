...
local choice_tilladseng = {function(player,choice)
  vRPclient.getNearestPlayer(player,{3},function(nplayer)
    ...
    if nuser_id ~= nil then
		...
		TriggerClientEvent("pNotify:SendNotification", player,{text = "Du tillod personen en sygeplads.", type = "error", queue = "global", timeout = 5000, layout = "centerLeft",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
		TriggerClientEvent("pNotify:SendNotification", nplayer,{text = "Du kan nu ligger dig i en seng.", type = "error", queue = "global", timeout = 5000, layout = "centerLeft",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
    else
		TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.common.no_player_near()}, type = "error", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
    end
  end)
    ...
    if user_id ~= nil then
        vRPclient.getNearestPlayer(player,{2},function(nplayer)
			...
			if vRP.hasPermission(nuser_id, "cop.keycard") then 
				...
				PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/606747425931132930/ciFwy82mCmUCeyOzjiespTTxfXMMJ-DfHZq8QA52ur0lWYs1CRA0WK-zfZfEbLKayDOh', function(err, text, headers) end, 'POST', json.encode({username = dname, content = dmessage}), { ['Content-Type'] = 'application/json' })
			else
						if nuser_id ~= nil then
							vRPclient.isInComa(nplayer,{}, function(in_coma)
								if in_coma then
								...
								if handcuffed then
									table.insert(seized_guns, "**"..cop_name.." "..cop_lname.." ("..user_id..")** beslaglagde følgende våben fra **"..civ_name.." "..civ_lname.." ("..nuser_id..")**: \n")
									  vRPclient.replaceWeapons(nplayer,{},function(weapons)
										...
										vRPclient.giveWeapons(nplayer,{{},true})
										PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/606747086565933066/EZuKmZHE803_ksNpWICib075spOIz3a9Hqj2GriACq_Jv2ZQVKnHCBpJcmGGEx8dfaQC', function(err, text, headers) end, 'POST', json.encode({username = "HomeBase (våben) - SRV " .. GetConvar("servernumber","0"), content = table.concat(seized_guns)}), { ['Content-Type'] = 'application/json' })
						...
						vRP.getUserIdentity(nuser_id, function(identity_civ)
							if identity_cop and identity_civ then
								...
								if handcuffed then
									...
									PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/606747531141185546/0DlgGwyZ3dkIEHa_kdvbkFHiTP6q-sRbNj5HFwwoLkcukcvIy9XhO5HsnaucaIqb9bJ6', function(err, text, headers) end, 'POST', json.encode({username = "Sky-Universe Politi - SRV " .. GetConvar("servernumber","0"), content = table.concat(seized_items)}), { ['Content-Type'] = 'application/json' })
...
local choice_store_weapons = {function(player, choice)
	...
	if user_id ~= nil then
		vRPclient.replaceWeapons(player,{},function(weapons)
          ...
          TriggerClientEvent('3dme:triggerDisplay', -1, "gemmer genstand", player)
		  TriggerClientEvent('kaz_attachments:return', player)
					for k,v in pairs(weapons) do
					...
					PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/606747086565933066/EZuKmZHE803_ksNpWICib075spOIz3a9Hqj2GriACq_Jv2ZQVKnHCBpJcmGGEx8dfaQC', function(err, text, headers) end, 'POST', json.encode({username = "Server " .. GetConvar("servernumber","0"), content = table.concat(stored_items)}), { ['Content-Type'] = 'application/json' })
					-- clear all weapons
...
local revive_seq = {
  ...
  {"mini@cpr@char_a@cpr_def","cpr_pumpchest_idle",1},
  {"amb@medic@standing@kneel@exit","exit",1}
}
          ...
          if in_coma then
            if vRP.tryGetInventoryItem(user_id,"medkit",1,true) then
              ...
              SetTimeout(10000, function()
				...
				TriggerClientEvent("pNotify:SendNotification", nplayer,{text ="Du er blevet genoplivet af en EMS-Job.", type = "success", queue = "global",timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})
              ...
              SetTimeout(7500, function()
				vRPclient.isInComa(nplayer,{}, function(in_coma2)
					if not in_coma2 then
						...
						TriggerClientEvent("pNotify:SendNotification", nplayer,{text ="Du er blevet helbredt af en EMS-Job.", type = "success", queue = "global",timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})
...
local choice_alkotest = {function(player,choice)
  ...
  if user_id ~= nil then
    vRPclient.getNearestPlayer(player,{10},function(nplayer)
      local nuser_id = vRP.getUserId(nplayer)
      if nuser_id ~= nil then
		TriggerClientEvent("pNotify:SendNotification", player,{text = "Tester med alkometer", type = "error", queue = "global", timeout = 15000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
		TriggerClientEvent("pNotify:SendNotification", nplayer,{text = "Pust.....", type = "error", queue = "global", timeout = 15000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
		Citizen.Wait(15000)
			...
			if vRP.hasPermission(user_id2,"police.handcuff") then
				vRPclient.isHandcuffed(cplayer,{}, function(handcuffed)
					if handcuffed then
						...
						TriggerClientEvent("pNotify:SendNotification", source,{text = {"Personen blev sat i håndjern."}, type = "info", queue = "global", timeout = 3000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},sounds = {sources = {"handcuff.ogg"},volume = 0.25,conditions = {"docVisible"}}, killer = true})
						TriggerClientEvent("pNotify:SendNotification", cplayer,{text = {"Du blev sat i håndjern."}, type = "info", queue = "global", timeout = 3000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},sounds = {sources = {"handcuff.ogg"},volume = 0.25,conditions = {"docVisible"}}, killer = true})
						Citizen.Wait(3000)
						vRPclient.toggleHandcuff(cplayer,{})
					end
				end)
				...
				vRPclient.isHandcuffed(cplayer,{}, function(handcuffed)
					if handcuffed then 
						vRPclient.toggleHandcuff(cplayer,{})
						TriggerClientEvent('kaz_handjern:fjern', source)
						TriggerClientEvent("pNotify:SendNotification", source,{text = {"Stripsne blev fjernet."}, type = "info", queue = "global", timeout = 3000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
					else
						if vRP.tryGetInventoryItem(user_id2,"strip",1,true) then
						TriggerClientEvent('kaz_handjern:target', cplayer, source)
						TriggerClientEvent('kaz_handjern:player', source)
						TriggerClientEvent("pNotify:SendNotification", source,{text = {"Personen blev sat i strips."}, type = "info", queue = "global", timeout = 3000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},sounds = {sources = {"handcuff.ogg"},volume = 0.25,conditions = {"docVisible"}}, killer = true})
						TriggerClientEvent("pNotify:SendNotification", cplayer,{text = {"Du blev sat i strips."}, type = "info", queue = "global", timeout = 3000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},sounds = {sources = {"handcuff.ogg"},volume = 0.25,conditions = {"docVisible"}}, killer = true})
						Citizen.Wait(3000)
						vRPclient.toggleHandcuff(cplayer,{})
						end
					end
