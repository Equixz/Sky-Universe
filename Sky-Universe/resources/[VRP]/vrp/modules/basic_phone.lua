...
--Discord
local DiscordWebhook = 'https://khRamlovOgHoej.com/api/webhooks/606749305977569302/BaVjhniONm9PBx7gDiBruqercfp5co2K7A2ceGs9u7r6Delp1b49zY2am4fYR_cejLPY'
...
function vRP.sendServiceAlert(sender, service_name,x,y,z,msg)
  ...
  if service then
    local players = {}
      ...
      -- PerformHttpRequest(DiscordWebhook, function(Error, Content, Head) end, 'POST', json.encode({username = "Distrikt "..GetConvar("servernumber"), content ="Modtaget af Politi-Job? ✔️ | Opringer: **"..user_id.."** | "..msg}), { ['Content-Type'] = 'application/json' })
      end 
...
RegisterServerEvent('gcPhone:kaztest')
AddEventHandler('gcPhone:kaztest', function(display, phoneNumber)
local player = source
...
RegisterServerEvent('gcPhone:kaztest2')
AddEventHandler('gcPhone:kaztest2', function(id)
local player = source
local user_id = vRP.getUserId(player)
  if user_id ~= nil then
	local phone_directory = vRP.getPhoneDirectory(user_id)
