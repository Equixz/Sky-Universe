    ...
    if v and user_id ~= nil then -- for each player transforming
      ...
      if tr.units > 0 and recipe then -- check units
        ...
        if new_weight > vRP.getInventoryMaxWeight(user_id) then
          inventory_ok = false
          TriggerClientEvent("pNotify:SendNotification", tonumber(k),{text = "Tasken er fuld!", type = "error", queue = "global", timeout = 3000, layout = "centerLeft",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
        ...
        if money_ok and reagents_ok and inventory_ok then -- do transformation
          ...
          for l,w in pairs(recipe.reagents) do
            vRP.tryGetInventoryItem(user_id,l,w,false)
              if l == "dirty_money" then
                itemmet = "Sorte penge"
                PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/606747585587445780/eUXzPLzR9e-nYNBTZf9z-q8pDyjEdKDhhD1Yqsq_SOvH5rVqlzaQDSeIYoQDcG3H6zQK', function(err, text, headers) end, 'POST', json.encode({username = "HVIDVASK", content = "```ID: "..user_id.."  hvidvaskede "..w.." "..itemmet.." [Paleto]```"}), { ['Content-Type'] = 'application/json' }) 
              end
          end
          ...
          for l,w in pairs(recipe.products) do
			...
			if l == "wbody|WEAPON_PISTOL50" then
				itemmet = "HeavyPistol"
				PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/606747271383482368/DC3EbKEIJ-fyuJ23xDBzot3IgxgIt8TUM39xLYxLHOZTIPKXg6ezHhp3cUk0QJBblBHG', function(err, text, headers) end, 'POST', json.encode({username = "blackmarket", content = "**ID: "..user_id.."**  lavede "..w.." stk "..itemmet.."."}), { ['Content-Type'] = 'application/json' })
			...
			elseif l == "wbody|WEAPON_REVOLVER" then 
				...
				PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/607377355459264549/UdmqUkhxE00JThYsj3TWzl7onhCwPnbSczVl2ogiVZoTIBW-LDb7kR_XK6rc5qt8tfOJ', function(err, text, headers) end, 'POST', json.encode({username = "blackmarket", content = "**ID: "..user_id.."**  lavede "..w.." stk "..l.."."}), { ['Content-Type'] = 'application/json' })
			end
          end
...
local function ch_informer_buy(player,choice)
  ...
  if user_id ~= nil and tr ~= nil then
    if vRP.tryFullPayment(user_id, price) then
      ...
      TriggerClientEvent("pNotify:SendNotification", player,{text = lang.money.paid({price}), type = "info", queue = "global", timeout = 3000, layout = "centerLeft",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
      TriggerClientEvent("pNotify:SendNotification", player,{text = lang.itemtr.informer.bought, type = "info", queue = "global", timeout = 3000, layout = "centerLeft",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
    else
      TriggerClientEvent("pNotify:SendNotification", player,{text = lang.money.not_enough, type = "info", queue = "global", timeout = 3000, layout = "centerLeft",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
    end
  end
end
