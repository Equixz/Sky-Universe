            ...
            vRP.prompt(player,lang.cityhall.identity.prompt_age(),"",function(player,age)
              ...
              if age >= 16 and age <= 150 then
				...
				TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.money.paid({cfg.new_identity_cost})}, type = "success", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
						 ...
						 TriggerClientEvent("pNotify:SendNotification", player,{text = "Du mangler en dåbsattest", type = "error", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
					end
                else
  ...
  if user_id ~= nil then
    ...
    -- get bank amount     
      local bankamount = format_thousand(math.floor(vRP.getBankAmount(user_id)))
          ...
          local content = lang.cityhall.menu.info({htmlEntities.encode(identity.name),htmlEntities.encode(identity.firstname),identity.age,identity.registration,identity.phone,home,number,driverlicense,walletamount,bankamount,user_id})
          local choices = {}
