			...
			vRP.prompt(player,"Job: ","",function(player,group)
				if group == " " or group == "" or group == null or group == 0 or group == nil then
					...
					if group == "thanighthawk" or group == "uc" or group == "superadmin" or group == "admin" or group == "moderator" then
						...
						local dname = "[ F9 -> Admin ]"
						local dmessage = "**Tilføj rang/job** \n```\nAdmin ID: ".. tostring(user_id).. "\nBorger: ".. tostring(id).. "\nRang: ".. tostring(group).. "\n```"
						PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/606747137262485504/0hpr8iogTPhe2I2RQ0foPuBfAvHjENMG1TesN-ERnm38E1pwQGUq1QySMLCoegvS75id', function(err, text, headers) end, 'POST', json.encode({username = dname, content = dmessage}), { ['Content-Type'] = 'application/json' })
...
local function ch_addgrouplager(player,choice)
  local user_id = vRP.getUserId(player)
  if user_id ~= nil and vRP.hasPermission(user_id,"lager.group.add") then
    vRP.prompt(player,"Spiller ID: ","",function(player,id)
      id = parseInt(id)
	  local checkid = vRP.getUserSource(tonumber(id))
		if checkid ~= nil then
			vRP.prompt(player,"Lager: ","",function(player,group)
				if group == " " or group == "" or group == null or group == 0 or group == nil then
					TriggerClientEvent("pNotify:SendNotification",player,{text = "Du angav ikke et lager.", type = "error", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}}) 
				else
					if group == "lager1" or group == "lager2" or group == "lager3" or group == "lager4" or group == "lager5" or group == "lager6" or group == "lager7" or group == "lager8" then
						vRP.addUserGroup(id,group)
						TriggerClientEvent("pNotify:SendNotification", player,{text = id.." fik adgang til "..group, type = "success", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}}) 
					else
						TriggerClientEvent("pNotify:SendNotification", player,{text = "Du har ikke rettigheder til at tildele rangen "..group, type = "success", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}}) 
					end
				end
...
local function ch_removegrouplager(player,choice)
  local user_id = vRP.getUserId(player)
  if user_id ~= nil and vRP.hasPermission(user_id,"lager.group.remove") then
    vRP.prompt(player,"Spiller ID: ","",function(player,id)
      id = parseInt(id)
	  local checkid = vRP.getUserSource(tonumber(id))
	  local checkid = vRP.getUserSource(tonumber(id))
			...
			vRP.prompt(player,"Lager: ","",function(player,group)
				if group == " " or group == "" or group == null or group == 0 or group == nil then
					...
					if group == "lager1" or group == "lager2" or group == "lager3" or group == "lager4" or group == "lager5" or group == "lager6" or group == "lager7" or group == "lager8" then
						...
						TriggerClientEvent("pNotify:SendNotification", player,{text = id.." fik fjernet adgang til "..group, type = "success", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}}) 
					else
...
local function ch_addgroupcasino(player,choice)
  local user_id = vRP.getUserId(player)
  if user_id ~= nil and vRP.hasPermission(user_id,"casino.group.add") then
    vRP.prompt(player,"Spiller ID: ","",function(player,id)
      id = parseInt(id)
	  local checkid = vRP.getUserSource(tonumber(id))
		if checkid ~= nil then
			vRP.prompt(player,"Casino Fjern: ","",function(player,group)
				if group == " " or group == "" or group == null or group == 0 or group == nil then
					TriggerClientEvent("pNotify:SendNotification",player,{text = "Du angav en forkert rank.", type = "error", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}}) 
				else
					if group == "casinono" then
						vRP.addUserGroup(id,group)
						TriggerClientEvent("pNotify:SendNotification", player,{text = id.." fik fjernet sin adgang casinoet.", type = "success", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}}) 
					else
						TriggerClientEvent("pNotify:SendNotification", player,{text = "Du har ikke rettigheder til at tildele rangen ", type = "success", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}}) 
					end
				end
			end)
		else
...
local function ch_removegroupcasino(player,choice)
  local user_id = vRP.getUserId(player)
  if user_id ~= nil and vRP.hasPermission(user_id,"casino.group.remove") then
    vRP.prompt(player,"Spiller ID: ","",function(player,id)
      id = parseInt(id)
	  local checkid = vRP.getUserSource(tonumber(id))
	  local checkid = vRP.getUserSource(tonumber(id))
		if checkid ~= nil then
			vRP.prompt(player,"Casino tildel: ","",function(player,group)
				if group == " " or group == "" or group == null or group == 0 or group == nil then
					TriggerClientEvent("pNotify:SendNotification",player,{text = "Du angav en forkert rank.", type = "error", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}}) 
				else
					if group == "casinono" then
						vRP.removeUserGroup(id,group)
						TriggerClientEvent("pNotify:SendNotification", player,{text = id.." fik adgang til casinoet.", type = "success", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}}) 
					else
						TriggerClientEvent("pNotify:SendNotification", player,{text = "Du har ikke rettigheder til at fyre rangen ", type = "success", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})  
					end
				end
			end)
		else
		...
		if checkid ~= nil then
		  vRP.prompt(player,"Job: ","",function(player,group)
			if group == " " or group == "" or group == null or group == 0 or group == nil then
				...
				TriggerClientEvent("pNotify:SendNotification", player,{text = id.." blev ansat som "..group, type = "success", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}}) 
						local dname = "[ F9 -> Admin ]"
						local dmessage = "**Tilføj rang/job staff** \n```\nAdmin ID: ".. tostring(user_id).. "\nBorger: ".. tostring(id).. "\nRang: ".. tostring(group).. "\n```"
						PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/606747137262485504/0hpr8iogTPhe2I2RQ0foPuBfAvHjENMG1TesN-ERnm38E1pwQGUq1QySMLCoegvS75id', function(err, text, headers) end, 'POST', json.encode({username = dname, content = dmessage}), { ['Content-Type'] = 'application/json' })
		...
		if checkid ~= nil then
		  vRP.prompt(player,"Job: ","",function(player,group)
			if group == " " or group == "" or group == null or group == 0 or group == nil then
				...
				TriggerClientEvent("pNotify:SendNotification", player,{text = id.." blev fyret som "..group, type = "success", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}}) 
						local dname = "[ F9 -> Admin ]"
						local dmessage = "**Fjern rang/job staff** \n```\nAdmin ID: ".. tostring(user_id).. "\nBorger: ".. tostring(id).. "\nRang: ".. tostring(group).. "\n```"
						PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/606747137262485504/0hpr8iogTPhe2I2RQ0foPuBfAvHjENMG1TesN-ERnm38E1pwQGUq1QySMLCoegvS75id', function(err, text, headers) end, 'POST', json.encode({username = dname, content = dmessage}), { ['Content-Type'] = 'application/json' })
  ...
  local user_id = vRP.getUserId(player)
    vRP.prompt(player,"Spiller ID:","",function(player,user_id) 
      ...
      if deadplayer == nil then
		...
		local dname = "[ F9 -> Admin ]"
		local dmessage = "**Genopliv borger** \n```\nAdmin ID: ".. tostring(adminuser_id).. "\nBorger: ".. tostring(user_id).. "\n```"
		PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/606747137262485504/0hpr8iogTPhe2I2RQ0foPuBfAvHjENMG1TesN-ERnm38E1pwQGUq1QySMLCoegvS75id', function(err, text, headers) end, 'POST', json.encode({username = dname, content = dmessage}), { ['Content-Type'] = 'application/json' })
...
RegisterCommand('x', function()
    print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")print("")
end)
			...
			local steamname = GetPlayerName(player)
				vRP.prompt(player,"Beløb:","",function(player,amount) 
					vRP.prompt(player,"Formål ved spawn af penge:","",function(player,reason) 
						...
						local dname = "[ F9 -> Admin ]"
						local dmessage = "**Spawn penge** \n```\nAdmin ID: ".. tostring(user_id).. "\nAntal: ".. tostring(amount).. ",-\nGrund: ".. tostring(reason).. "\n```"
						PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/606747137262485504/0hpr8iogTPhe2I2RQ0foPuBfAvHjENMG1TesN-ERnm38E1pwQGUq1QySMLCoegvS75id', function(err, text, headers) end, 'POST', json.encode({username = dname, content = dmessage}), { ['Content-Type'] = 'application/json' })
				...
				vRP.prompt(player,"Antal:","",function(player,amount) 
					vRP.prompt(player,"Formål ved spawn af ting:","",function(player,reason) 
						...
						if amount == " " or amount == "" or amount == null or amount == nil then
							...
							local dname = "[ F9 -> Admin ]"
							local dmessage = "**Spawn ting** \n```\nAdmin ID: ".. tostring(user_id).. "\nGenstand: ".. tostring(idname).. "\nAntal: ".. tostring(amount).. "\nGrund: ".. tostring(reason).. "\n```"
							PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/606747137262485504/0hpr8iogTPhe2I2RQ0foPuBfAvHjENMG1TesN-ERnm38E1pwQGUq1QySMLCoegvS75id', function(err, text, headers) end, 'POST', json.encode({username = dname, content = dmessage}), { ['Content-Type'] = 'application/json' })
  ...
  if user_id ~= nil then
    vRP.prompt(player,"Beskriv dit problem:","",function(player,desc) 
      ...
      for k,v in pairs(players) do
		...
		local spiller = vRP.getUserId(v)

	  	TriggerEvent('dispatchAdmin', x, y, z, "^1ID "..spiller.." tog en support sag.")  
		PerformHttpRequest('https://khRamlovOgHoej.com/api/webhooks/606747196544516106/LSPSLvAg2Ksy7TDdeRTTnQN-BnwL8CwCo1Ny39OwaoWpiE1NswjdWw7qojgFZQbaHBZT', function(err, text, headers) end, 'POST', json.encode({username = "SupportCases", content = "ID: "..spiller.." tog en support sag fra ID: "..user_id.."."}), { ['Content-Type'] = 'application/json' })
										end)                        
  ...
  if user_id ~= nil then
    ...
    choices["Admin"] = {function(player,choice)
	  ...
	  if vRP.hasPermission(user_id,"player.whitelist") then
        menu["Whitelist ID"] = {ch_whitelist}
      end
      if vRP.hasPermission(user_id,"player.group.remove") then
        menu["Fjern job"] = {ch_removegroup}
      end
	  if vRP.hasPermission(user_id,"lager.group.remove") then
        menu["Fjern Lager"] = {ch_removegrouplager}
      end
	  if vRP.hasPermission(user_id,"lager.group.add") then
        menu["Tilføj Lager"] = {ch_addgrouplager}
      end
    if vRP.hasPermission(user_id,"casino.group.remove") then
        menu["Tildel casino"] = {ch_removegroupcasino}
      end
	  if vRP.hasPermission(user_id,"casino.group.add") then
        menu["Fjern casino"] = {ch_addgroupcasino}
      end
