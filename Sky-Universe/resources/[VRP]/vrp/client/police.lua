...
Citizen.CreateThread(function()
  while true do
    Citizen.Wait(0)
		if IsControlJustReleased(1,213) then
  ...
  while true do
    ...
    if car ~= 0 and (wasInCar or IsCar(car)) then
      ...
      if (IsPedInAnyVehicle(ped, false)) and beltOn == true then      
		  drawTxt(0.663, 1.42, 1.0,1.0,0.6, "~b~S", 255, 255, 255, 255)
		else
		  drawTxt(0.663, 1.42, 1.0,1.0,0.6, "~r~S", 255, 255, 255, 255)
      end
...
function tvRP.putInNearestVehicleAsPassenger(radius)
  local veh = tvRP.getNearestVehicle2(radius)
  if IsEntityAVehicle(veh) then
    kazcar = GetVehicleMaxNumberOfPassengers(veh)
	if kazcar > 3 then kazcar = 3 end
		if kazcar == 1 then 
			if IsVehicleSeatFree(veh,0) then
				SetPedIntoVehicle(GetPlayerPed(-1),veh,0)
				beltOn = false
			end
		elseif kazcar == 3 then 
			if IsVehicleSeatFree(veh,2) then
				SetPedIntoVehicle(GetPlayerPed(-1),veh,2)
				beltOn = false
