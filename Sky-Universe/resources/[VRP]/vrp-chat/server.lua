                ...
                if args[3] ~= nil then
                    ...
                    --TriggerClientEvent('vrp-chat:notify',checkid, "OOC ^7| ^1STAFF ^7| ^3SVAR TIL DIG ^7| " ..author..": ^1" ..  smessage)
                            TriggerClientEvent("pNotify:SendNotification", checkid, {
                                text = "<b style = 'color:black'><center><h2> "..smessage.." </h2><br>Sendt af "..author.."</b>",
                                type = "alert",
                                theme = "alarm",
                                queue = "lmao",
                                timeout = 15000,
    ...
    elseif string.lower(args[1]) == "/ooc" then
        ...
        if vRP.hasPermission({user_id,cfg.notifyStaffPerm}) then
            ...
            if message ~= "" then
                ...
                TriggerClientEvent('chatMessage',-1, "OOC ^7| ^1STAFF ^7| " ..author..": ^7" ..  message)
            end
        end
        CancelEvent()
    elseif string.lower(args[1]) == "/ck" and cfg.ckCommand then
        local message = string.sub(msg, 5)
        local type = string.upper(string.sub(msg,2,3))
        if message ~= "" then
            local user_id = vRP.getUserId({source})
            sendToDiscord(type.." CHAT", "**"..tostring(user_id) .. "** - " ..author..": ^7" .. message)
            TriggerClientEvent('chatMessage',-1, "^5"..type.." ^7| ^3SPILLER: ^7" .. message)
        end
        CancelEvent()
    end
end)
...
AddEventHandler('vrp-chat:chat_message', function(source,author,message)
    ...
    if vRP.hasPermission({user_id,cfg.notifyStaffPerm}) then
        ...
        TriggerClientEvent('chatMessage', source, "OOC ^7| ^5SPILLER ^7| "..user_id.." ^7| " ..author..": ^7" ..  message)
    end
    local users = vRP.getUsers({})
    for k,v in pairs(users) do
        if v ~= nil then
