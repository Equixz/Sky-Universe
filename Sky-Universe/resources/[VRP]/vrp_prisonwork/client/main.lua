...
AddEventHandler('vRP:hasEnteredMarker', function (zone)
  if zone == 'StartJob' then
	...
	CurrentActionMsg  = ('Tryk ~INPUT_CONTEXT~ for at lave samle vasketøj.')
    CurrentActionData = {}
  elseif zone == 'LeaveBox' then
    CurrentAction	  = 'leave_box'
	CurrentActionMsg  = ('Tryk ~INPUT_CONTEXT~ for at aflevere vasketøjet.')
	CurrentActionData = {}
  end
end)
  ...
  if CurrentAction ~= nil then
    ...
    if IsControlPressed(0,  Keys['E']) then
		...
		CurrentAction = nil
			...
			text = ('Sikkerhedsvagt  »  Du har ingen pakke.'),
			type = "error",
			timeout = (3500),
			layout = "bottomCenter",
			queue = "loffe",
  ...
  while true do
    ...
    for k,v in pairs(Config.Zones) do
      if(v.Type ~= -1 and GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Config.DrawDistance) then
        DrawMarker(v.Type, v.Pos.x, v.Pos.y, v.Pos.z, 0, 0, 0, 0, 0, 0, 1.2, 1.2, 1.0, v.Color.r, v.Color.g, v.Color.b, 200, 0, 0, 1, 0.5)
      end
    end
  end
end)
...
AddEventHandler('vRP:startJob', function()
	...
	SetEntityCoords(GetPlayerPed(-1), 1664.7800292969,2593.0764160156,45.587387084961-0.95) 
	SetEntityHeading(GetPlayerPed(-1), 353.76620483398)
    TaskStartScenarioInPlace(GetPlayerPed(-1), "PROP_HUMAN_BUM_BIN", 0, true)
	CanLeaveBox = true
	TimeLeft = Config.TimeToPackBox
	repeat
	TriggerEvent("vRP:missiontext", '~w~Samler Vasketøj. Tid tilbage: ~r~' .. TimeLeft .. ' ~w~Sekunder', 1000)
	TimeLeft = TimeLeft - 1
	Citizen.Wait(1000)
	until(TimeLeft == 0)
