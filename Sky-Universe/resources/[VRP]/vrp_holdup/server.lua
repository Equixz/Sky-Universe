...
AddEventHandler('vrp_holdup:rob', function(robb,x,y,z)
  ...
  if vRP.hasPermission({user_id,cfg.permission}) then
	TriggerClientEvent("pNotify:SendNotification", player,{text = "Du kan ikke røve butikken som Politi-Job.", type = "error", queue = "global", timeout = 5000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})
  else
	  ...
	  if stores[robb] then
		  ...
		  SetTimeout(cfg.seconds*1000, function()
			  if(robbers[savedSource])then
				  if(user_id)then
					TriggerClientEvent('kaz_holdup:completecheck', savedSource, store.reward, store.nameofstore, savedSource, user_id)
				  end
			  end
		  end)		
	  end
    else
     TriggerClientEvent("pNotify:SendNotification", player,{text = "Der skal være min. <b>" ..cfg.cops.."</b> Politi-Jobe på, før du kan røve denne butik", type = "error", queue = "global", timeout = 5000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
    end
  end
end)

RegisterServerEvent('kaz:payupbug')
AddEventHandler('kaz:payupbug', function()
local source = source
local user_id = vRP.getUserId({source})
vRP.giveInventoryItem({user_id,"dirty_money",15000,true})
TriggerClientEvent("pNotify:SendNotification", -1,{text = "Røverne slap væk.", type = "warning", queue = "global", timeout = 5000, layout = "bottomCenter",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
TriggerClientEvent('vrp_holdup:robberycomplete', source, 15000)
end)


RegisterServerEvent('kaz_holdup:completecheck2')
AddEventHandler('kaz_holdup:completecheck2', function(storereward, storenameofstore, savedSource, user_id)
