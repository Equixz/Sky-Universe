...
local fareCost = 2 --(1.66 = $100 per minute) Cost per second
local costPerMile = 65
    ...
    while true do
      ...
      if(IsInTaxi()) then
        if (inTaxi == false) then
          TriggerEvent("pNotify:SendNotification",{text = "Tryk <b>N</b> for at starte Taximeteret",type ="info",timeout=(2000),layout ="bottomCenter",queue ="taxi",animation ={open ="gta_effects_fade_in",close ="gta_effects_fade_out"}})
     end
        inTaxi = true
        local ped = GetPlayerPed(-1)
        local veh = GetVehiclePedIsIn(ped, false)
        if(IsInTaxi() and GetPedInVehicleSeat(veh, -1) == ped) then
          if IsControlJustPressed(1, 249)  then -- IF INPUT_PICKUP Is pressed
              if meterOpen then
                closeGui()
                meterOpen = false
              else
