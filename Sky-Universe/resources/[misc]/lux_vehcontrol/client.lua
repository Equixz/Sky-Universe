			...
			if IsPedInAnyVehicle(playerped, false) then	
				...
				if GetPedInVehicleSeat(veh, -1) == playerped then
					...
					if GetVehicleClass(veh) == 18 then
						...
						DisableControlAction(0, 158, true) -- INPUT_VEH_NEXT_RADIO
						DisableControlAction(0, 157, true) -- INPUT_VEH_PREV_RADIO
						--DisableControlAction(0, 82, true) -- INPUT_VEH_NEXT_RADIO
						DisableControlAction(0, 81, true) -- INPUT_VEH_PREV_RADIO
						...
						if not IsPauseMenuActive() then
							...
							if state_lxsiren[veh] > 0 then
								if IsDisabledControlJustReleased(0, 158) or IsDisabledControlJustReleased(0, 81) then
									if IsVehicleSirenOn(veh) then
										local cstate = state_lxsiren[veh]
										local nstate = 1
										PlaySoundFrontend(-1, "NAV_LEFT_RIGHT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1) -- on
							...
							if state_lxsiren[veh] < 1 then
								if IsDisabledControlPressed(0, 158) or IsDisabledControlPressed(0, 81) then
									actv_manu = true
								else
									actv_manu = false
								end
