...
admins = {
    'steam:1100001000c2664',
    'steam:110000104e74173',
    'steam:110000100eef108',
    'steam:110000102406f15',
	'steam:11000010267fd95',
...
RegisterCommand('empthisshit', function(source)
...
-- Af en eller anden grund - vil den gerne calle MIT ID ( 1 ) istedet for jonas.
	...
	local nuser_id = vRP.getUserId({source})
		if nuser_id ~= nil then
			vRP.getUserIdentity({nuser_id, function(identity)
				blackout = not blackout
				if blackout then
					if vRP.hasInventoryItem({nuser_id, "emp"}) then
						TriggerClientEvent('vSync:notify', source, 'Du trykkede på ~b~EMPen~s~.')
						TriggerEvent('vSync:requestSync')
					end
				else
					if vRP.hasInventoryItem({nuser_id, "emp"}) then
