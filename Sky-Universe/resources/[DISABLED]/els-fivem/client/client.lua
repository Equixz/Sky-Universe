...
RegisterNetEvent("kaz_els:kazoonspecial")
AddEventHandler("kaz_els:kazoonspecial", function()
Citizen.Wait(10000)
if elsactive == true then 
	elsactive = false
	TriggerEvent("pNotify:SendNotification",{text = "ELS er blevet slået fra",type = "success",timeout = (8500),layout = "centerRight",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
end
end)
	...
	while true do 
		Citizen.Wait(0)
			if IsControlJustPressed(1, 168) then
				if elsactive == true then 
					elsactive = false
					TriggerEvent("pNotify:SendNotification",{text = "Els slået fra!",type = "success",timeout = (8500),layout = "centerRight",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
				else
					elsactive = true
					TriggerEvent("pNotify:SendNotification",{text = "Els slået til!",type = "success",timeout = (8500),layout = "centerRight",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
				end
			end
	end
end)
